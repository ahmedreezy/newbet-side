    ...
    <div v-else>
      ...
      <div v-else class="media-grid">
        <div v-for="item in testimonials" :key="item.id" class="media-item">
          ...
          <template v-else>
            <img v-if="imageSrc(item)" :src="imageSrc(item)" class="media-img" :alt="item.caption || 'Testimonial'" />
...
.media-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; margin-bottom: 30px; }
.media-item { background: #111; border: 1px solid rgba(255, 215, 0, 0.12); border-radius: 14px; overflow: hidden; position: relative; }
.media-img { width: 100%; aspect-ratio: 3 / 4; object-fit: cover; display: block; }
.media-placeholder { aspect-ratio: 3 / 4; display: flex; align-items: center; justify-content: center; color: #666; background: #080808; }
