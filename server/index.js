...
async function start() {
  const { pool: dbPool, isInMemory } = require('./db')
  const { migrate } = require('./migrate')
  const { seed }    = require('./seed')
  ...
  if (!process.env.DATABASE_URL) {
    ...
    try {
      await migrate()
      // Restore persisted data before seeding so seed doesn't overwrite real data
      await restoreSnapshot(dbPool)
      // Only seed if admin_users is empty (first ever run)
      const { rows } = await dbPool.query('SELECT COUNT(*) FROM admin_users')
      if (parseInt(rows[0].count, 10) === 0) await seed()
    } catch (err) {
      console.error('⚠ DB init warning:', err.message)
